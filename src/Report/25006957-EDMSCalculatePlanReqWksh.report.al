report 25006957 "EDMS Calculate Plan-Req. Wksh."//699
{
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(KeepExistingLines; KeepExistingLines)
                    {
                        ApplicationArea = Basic;
                        Caption = 'Keep Existing Lines';
                    }
                }
            }
        }
    ...
    var
        SkipPlanning: Boolean;
        IsHandled: Boolean;
        GlobalSKU: Record "General Stockkeeping Unit";
    begin
        IsHandled := false;
        SkipPlanning := false;
...
then
        ...
        //10.05.2012. EDMS P8 >>
        GlobalSKU.Reset;
        Item.Copyfilter("Location Filter", GlobalSKU."Location Code");
        GlobalSKU.SetRange("Item Category Code", Item."Item Category Code");
        if GlobalSKU.FindFirst then
            exit(false);
        //10.05.2012. EDMS P8 <<
        exit(SkipPlanning);
    end;
