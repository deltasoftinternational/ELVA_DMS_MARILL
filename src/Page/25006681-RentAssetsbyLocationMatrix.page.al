page 25006681 "Rent Assets by Location Matrix"
{
    Caption = 'Rent Assets by Location Matrix';
    Editable = false;
    LinksAllowed = false;
    PageType = ListPart;
    ...
    actions
    {
        area(processing)
        {
            group("&RentAsset")
            {
                Caption = '&Rent Asset';
                Image = Item;
                group("&Rent Asset Availability by")
                {
                    Caption = '&Rent Asset Availability by';
    ...
    var
        ...
        RentAssetFilterRec: Record "Rent Asset";
        RentRelationFilterRec: Record "Rent Item Relation";
        FindAssetsByRelation: Boolean;
        [InDataSet]
    ...
    var
        TempItem: Record "Rent Asset" temporary;
    begin
        TempItem.Copy(Rec);
        TempItem.SetRange("Location Filter", MatrixRecords[ColumnID].Code);
        TempItem.CalcFields(Inventory);
        MATRIX_CellData[ColumnID] := TempItem.Inventory - GetAllocatedAssetQuantity(Rec."No.", MatrixRecords[ColumnID].Code);
        SetVisible;
    end;
    ...
    begin
        if OnlyAvailabe then
            Exit;
        RentLedgerEntry.SetCurrentKey(
          "Rent Asset No.", "Entry Type", "Location Code", "Posting Date");
        RentLedgerEntry.SetRange("Rent Asset No.", Rec."No.");
        RentLedgerEntry.SetRange("Location Code", MatrixRecords[ColumnID].Code);
        PAGE.Run(0, RentLedgerEntry);
    ...
    procedure SetOnlyAvailable(OnlyAvailableToSet: Boolean)
    begin
        OnlyAvailabe := OnlyAvailableToSet;
    end;

    procedure SetOnlyMultiple(OnlyMultipleToSet: Boolean)
    begin
        OnlyMultiple := OnlyMultipleToSet;
    end;

    procedure SetRentAssetFilter(var RentAssetFilterToSet: Record "Rent Asset")
    begin
        FindAssetsByRelation := false;
        RentAssetFilterRec.Copy(RentAssetFilterToSet);
    end;

    procedure SetRentRelationFilter(var RentRelationFilterToSet: Record "Rent Item Relation")
    begin
        FindAssetsByRelation := true;
        RentRelationFilterRec.Copy(RentRelationFilterToSet);
    end;

    procedure GetAllocatedAssetQuantity(RentAssetNo: Code[20]; LocationCode: Code[20]): Decimal
    var
        RentLine: Record "Rent Line";
        AllocatedQty: Decimal;
    begin
        if not OnlyAvailabe then
            Exit(0);
        RentLine.SetRange("Rent Asset No.", RentAssetNo);
        RentLine.SetRange(Status, RentLine.Status::Allocated);
        RentLine.SetRange("Location Code", LocationCode);
        if RentLine.FindFirst then
            repeat
                AllocatedQty += RentLine."Rent Asset Quantity";
    ...
    begin
        if FindAssetsByRelation then begin
            RentAssetFilterRec.Reset();
            if RentRelationFilterRec.FindSet then
                repeat
                    if RentAssetFilterRec.Get(RentRelationFilterRec."Rent Asset No.") then begin
                        if OnlyMultiple then begin
                            if RentAssetFilterRec."Asset Type" = RentAssetFilterRec."Asset Type"::Multiple then
                                RentAssetFilterRec.Mark(true);
                        end else begin
                            RentAssetFilterRec.Mark(true);
                        end;
                    end;
                until RentRelationFilterRec.Next = 0;

            RentAssetFilterRec.MarkedOnly(true);
        end else begin
            if RentAssetFilterRec.FindSet() then
                repeat
                    if OnlyMultiple then begin
                        if RentAssetFilterRec."Asset Type" = RentAssetFilterRec."Asset Type"::Multiple then
                            RentAssetFilterRec.Mark(true);
                ...
                until RentAssetFilterRec.Next() = 0;
            RentAssetFilterRec.MarkedOnly(true);
        end;
        CurrPage.SetTableView(RentAssetFilterRec);
    end;
}
