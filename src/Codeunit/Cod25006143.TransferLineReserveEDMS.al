codeunit 25006143 "Transfer Line-Reserve EDMS"
{
    ...
    procedure FindReservEntrySet(TransLine: Record "Transfer Line"; var ReservEntry: Record "Reservation Entry"; Direction: Enum "Transfer Direction"): Boolean
    begin
        ReservEntry.InitSortingAndFilters(false);
        TransLine.SetReservationFilters(ReservEntry, Direction);
    ...
    begin
        ...
        for Direction := Direction::Outbound to Direction::Inbound do begin
            ...
            if NewTransLine."Qty. per Unit of Measure" <> OldTransLine."Qty. per Unit of Measure" then
                ReservMgt.ModifyUnitOfMeasure();
            ReservMgt.DeleteReservEntries(false, NewTransLine."Outstanding Qty. (Base)");
    ...
    begin
        if not TempTrackingSpecification.FindSet() then
            exit;
        TransferLineReserve.InitFromTransLine(SourceTrackingSpecification, TransferLine, TransferLine."Shipment Date", Direction::Outbound);
